...
namespace Amigo.Application.Validation.Common.Specifications
{
    public static class OrderCommonSpecifciation
    {
       ...
       (GetAllOrdersQuery query, string userId)
        ...
        public static Expression<Func<Order, bool>> BuildCriteriaForAdmin
            (GetAllAdminOrdersQuery query)
        {
            ...
            return o =>
                    !o.IsDeleted &&
                    ( string.IsNullOrWhiteSpace(query.UserName) || o.User.FullName.Trim().ToLower().Contains(query.UserName))
                    &&
                    (string.IsNullOrWhiteSpace(query.UserEmail) || o.User.Email == query.UserEmail)
